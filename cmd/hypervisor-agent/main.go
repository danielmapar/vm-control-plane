...
func parseFlags() options {
	var (
		...
		pinCPUSet    = flag.String("pin-cpuset", "", "libvirt: pin guest vCPU+emulator threads to these host cores (e.g. 8-15)")
		pollInterval = flag.Duration("poll-interval", 0, "work-claim poll interval (0 = default 300ms)")
		emulated     = flag.Bool("emulated", false, "libvirt: run guests under QEMU TCG software emulation instead of hardware KVM (slower; needs no /dev/kvm)")
	)
	flag.Parse()
